...
<?php else: ?>
    <div class="congrats-panel"><p class="eyebrow">Election complete</p><h2>Congratulations to the elected representatives.</h2><div class="winner-summary"><?php foreach ($results['positions'] as $position): ?><div><strong><?= htmlspecialchars($position['name'], ENT_QUOTES, 'UTF-8') ?></strong><span><?php if ($position['winners']): ?><?= htmlspecialchars(implode(' and ', array_map(static fn (array $winner): string => $winner['first_name'] . ' ' . $winner['last_name'], $position['winners'])), ENT_QUOTES, 'UTF-8') ?><?php else: ?>No winner recorded<?php endif; ?></span></div><?php endforeach; ?></div><a class="button button-primary" href="/results/export">Export winners</a></div>
